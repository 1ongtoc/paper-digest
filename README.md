...
- IACR ePrint：通过官方 OAI-PMH 分页回补投递窗口内的全部首次发布论文；未命中兴趣词时保留英文元数据，命中后与 arXiv 使用相同分级——AI 判定不相关时忠实翻译标题和原始摘要，相关论文生成摘要导读，直接安全推理论文生成全文精选。OAI 临时不可用时降级到 recent RSS，但不推进回补游标。
